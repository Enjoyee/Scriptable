...
class Widget extends BaseWidget {
  ...
  redPackageInfoFun = async () => {
    const url = `https://wq.jd.com/user/info/QueryUserRedEnvelopesV2${this.joinQueryParams(this.defaultPreference.redPackageParam)}`;
    const options = {
      headers: {
        ...this.defaultHeaders,
    ...
    if (RES?.errcode == '0') {
      const { data } = RES;
      this.redPackage.amount = data.balance ? data.balance : 0;
      if (data.expiredBalance && data.expiredBalance !== '') {
        this.redPackage.expired = data.expiredBalance;
      }
    } else {
    ...
    console.log(`红包信息：`);
    console.log(
      JSON.stringify(this.redPackage, null, 2)
    );
    this.logDivider();
  }
